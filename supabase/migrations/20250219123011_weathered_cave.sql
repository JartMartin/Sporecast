...
ADD COLUMN role text CHECK (role IN ('admin', 'user'));
...
COMMENT ON COLUMN company_invites.role IS 'Role to be assigned to the invited user';
COMMENT ON COLUMN profiles.company_id IS 'Company the user belongs to';
COMMENT ON COLUMN profiles.role IS 'User role within the company (admin or user)';